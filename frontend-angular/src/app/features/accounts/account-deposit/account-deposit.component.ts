...
@Component({
  ...
  template: `
    <div class="container">
      ...
      @if (account) {
        <p class="muted">Account {{ account.accountNumber }} ({{ account.currency }})</p>
      }
      @if (form && accountId != null) {
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="card">
          ...
          <div class="form-group">
            ...
            <input id="description" type="text" formControlName="description" placeholder="e.g. Cash deposit" />
          </div>
...
export class AccountDepositComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.loading = true;
    this.accountService.deposit(this.accountId, {
      amount: Number(raw.amount),
      description: raw.description?.trim() || undefined,
    }).subscribe({
      next: (res) => {
        this.toast.showSuccess(`Deposit successful. New balance: ${res.balanceAfter.toFixed(2)}`);
        this.router.navigate(['/accounts', this.accountId]);
        this.loading = false;
      },
      error: (err: { error?: { message?: string; detail?: string }; message?: string }) => {
        const msg = err.error?.message ?? err.error?.detail ?? err.message ?? 'Deposit failed';
        this.toast.showError(msg);
        this.loading = false;
      },
    });
