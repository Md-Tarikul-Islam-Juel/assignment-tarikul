...
export class PrismaService extends PrismaClient {
  ...
  private static getDatabaseUrl(config: ConfigService): string {
    ...
    // Construct DATABASE_URL from individual components (MySQL)
    const host = config.get<string>('DATABASE_HOST') || 'localhost';
    const port = config.get<number>('DATABASE_PORT') || 3306;
    const user = config.get<string>('DATABASE_USER') || 'root';
    const password = config.get<string>('DATABASE_PASSWORD') || '';
    const database = config.get<string>('DATABASE_NAME') || 'nest';
