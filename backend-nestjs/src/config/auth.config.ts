...
export default registerAs('authConfig', () => ({
  ...
  token: {
    useJwe: process.env.USE_JWE?.toLowerCase() === 'true', // default false: use plain JWT so all modules validate consistently
  ...
  password: {
    ...
    requireLowercase: process.env.PASSWORD_REQUIRE_LOWERCASE ? process.env.PASSWORD_REQUIRE_LOWERCASE.toLowerCase() === 'true' : true,
    requireUppercase: process.env.PASSWORD_REQUIRE_UPPERCASE ? process.env.PASSWORD_REQUIRE_UPPERCASE.toLowerCase() === 'true' : true,
    requireNumbers: process.env.PASSWORD_REQUIRE_NUMBERS ? process.env.PASSWORD_REQUIRE_NUMBERS.toLowerCase() === 'true' : true,
    requireSpecialCharacters: process.env.PASSWORD_REQUIRE_SPECIAL_CHARACTERS ? process.env.PASSWORD_REQUIRE_SPECIAL_CHARACTERS.toLowerCase() === 'true' : true,
    disallowRepeating: process.env.PASSWORD_DISALLOW_REPEATING ? process.env.PASSWORD_DISALLOW_REPEATING.toLowerCase() === 'true' : true,
    disallowSequential: process.env.PASSWORD_DISALLOW_SEQUENTIAL ? process.env.PASSWORD_DISALLOW_SEQUENTIAL.toLowerCase() === 'true' : true,
    blacklistCommon: process.env.PASSWORD_BLACKLIST_COMMON ? process.env.PASSWORD_BLACKLIST_COMMON.toLowerCase() === 'true' : true,
    excludeUsername: process.env.PASSWORD_EXCLUDE_USERNAME ? process.env.PASSWORD_EXCLUDE_USERNAME.toLowerCase() === 'true' : true
  },
