...
export interface UserRepositoryPort {
   ...
   */
  findManyCustomers(opts: { search?: string; limit?: number }): Promise<Array<{ id: number; email: string; firstName: string | null; lastName: string | null }>>;

  /**
   * Get email addresses by user IDs. Returns a map of userId -> email (missing IDs are omitted).
   */
  findEmailsByIds(ids: number[]): Promise<Map<number, string>>;
