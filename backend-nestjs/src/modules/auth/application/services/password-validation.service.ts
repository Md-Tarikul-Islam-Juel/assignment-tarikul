...
export class PasswordValidationService {
   ...
   * @param usernameOrEmail - Optional username or email to check against (for excludeUsername validation)
   * @returns Object with isValid flag and error message if invalid
   */
  validatePassword(password: string, usernameOrEmail?: string): {isValid: boolean; error?: string} {
    ...
    // Exclude username/email check
    const excludeUsername = this.configService.get<boolean>('authConfig.password.excludeUsername', true);
    if (excludeUsername && usernameOrEmail) {
      const normalizedPassword = password.toLowerCase();
      const normalizedUsername = usernameOrEmail.toLowerCase();
      
      // Check if password contains username/email or vice versa
      if (normalizedPassword.includes(normalizedUsername) || normalizedUsername.includes(normalizedPassword)) {
        return {
          isValid: false,
      ...
      if (usernamePart && usernamePart.length >= 3) {
        if (normalizedPassword.includes(usernamePart) || usernamePart.includes(normalizedPassword)) {
          return {
            isValid: false,
            error: 'Password must not contain your username or email address'
          };
