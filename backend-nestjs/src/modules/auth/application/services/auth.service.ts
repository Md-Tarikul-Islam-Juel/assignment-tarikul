...
import type {
  ...
  SignupDto,
  VerificationDto,
} from '../../interface/dto/auth-request.dto';
import type {
...
export class AuthService {
  ...
  async createEmployee(dto: CreateEmployeeDto): Promise<{ id: number; email: string; role: string }> {
    const command = CreateStaffCommand.forEmployee(dto);
    return this.createStaffUseCase.execute(command);
  }
}
