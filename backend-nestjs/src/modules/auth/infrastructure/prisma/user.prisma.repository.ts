...
export class UserPrismaRepository implements UserRepositoryPort {
  ...
  /** Only CUSTOMER role (excludes Admin and Employee) for create-account suggestions. */
  async findManyCustomers(opts: { search?: string; limit?: number }): Promise<Array<{ id: number; email: string; firstName: string | null; lastName: string | null }>> {
  ...
  async findEmailsByIds(ids: number[]): Promise<Map<number, string>> {
