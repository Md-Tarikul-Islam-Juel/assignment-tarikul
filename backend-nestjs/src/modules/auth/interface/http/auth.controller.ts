...
export class AuthController {
  ...
  @ApiOperation({ summary: 'List customers for autocomplete', description: 'Returns only users with role CUSTOMER (Admin and Employee are excluded). For use when creating an account. Optional search by email.' })
  @ApiQuery({ name: 'q', required: false, description: 'Search by email (partial match)' })
  @ApiQuery({ name: 'limit', required: false, description: 'Max results (default 20, max 50)' })
  @ApiOkResponse({ description: 'List of customers (id, email, firstName, lastName)' })
  async listCustomers(
    @Query('q') q?: string,
    @Query('limit') limit?: string
  ): Promise<{ success: boolean; data: Array<{ id: number; email: string; firstName: string | null; lastName: string | null }> }> {
    const limitNum = limit ? Math.min(parseInt(limit, 10) || 20, 50) : 20;
    const data = await this.userService.listCustomers(q?.trim(), limitNum);
